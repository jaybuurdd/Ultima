...
const dualTrade = async (router1,router2,baseToken,token2,amount) => {
  ...
  try {
    ...
    const tx = await ultima.connect(owner).dualDexTrade(router1, router2, baseToken, token2, amount); //{ gasPrice: 1000000000003, gasLimit: 500000 }
    await tx.wait();
    inTrade = false;
    await lookForDualTrade();
  } catch (e) {
...
const setup = async () => {
  ...
  for (let i = 0; i < config.baseAssets.length; i++) {
    ...
    const balance = await assetToken.balanceOf(config.ultimaContract);
...
const logResults = async () => {
  console.log(`############# LOGS #############`);
    ...
    const interface = await ethers.getContractFactory('WETH9');
    const assetToken = await interface.attach(asset.address);
    balances[asset.address].balance = await assetToken.balanceOf(config.ultimaContract);
